**Inclusion and Exclusion Criteria - Develop dataset for main analyses
...
"{\super 3}States dropped=AR, NJ, OH: Large change in % of MLTSS btw 05 & 12." \ ///
...
********************************************************************************
********************************************************************************
*Compare the hospitalization rate before and after the "hospitalization definition" change
mat mat1=J(3,3,.)
forvalues n=1/3{
preserve
if `n'==1{
...
tab _has_hosp year, col
sum _has_hosp
mat mat1[1,`n']=r(N)
mat mat1[2,`n']=round(r(mean)*100,0.01)
sum hosp
mat mat1[3,`n']=round(r(mean)*100,0.01)
restore
}
mat list mat1
putdocx begin
putdocx pagebreak
putdocx paragraph, style(Heading1)
putdocx text ("Table: Hospitalization Rate Comparison Before and After the “Hospitalization definition” adjustment")
putdocx save `logpath'\main`ver'.docx, append
frmttable using `logpath'/main_tab1.rtf, statmat(mat1) sdec(0,0,0 \ 2,2,2\ 2,2,2) ///
colwidth(20,2,2,2) ///
rtitles("N" \ ///
"Hosp % before adj. (%)" \ ///
"Hosp % after adj. (%)")  ///
ctitles("","Overall","2005","2012") ///
note() ///
replace
wordconvert "`logpath'/main_tab1.rtf" "`logpath'/main_tab1.docx", replace
putdocx append `logpath'/main`ver'.docx `logpath'/main_tab1.docx, saving(`logpath'/main`ver', replace)
...
forvalues a=0/1{
sum _has_hosp if hcbs==`a'
mat mat2[1,2*`n'-1+`a']=r(N)
mat mat2[2,2*`n'-1+`a']=round(r(mean)*100,0.01)
sum hosp if hcbs==`a'
mat mat2[3,2*`n'-1+`a']=round(r(mean)*100,0.01)
}
restore
}
mat list mat2

frmttable using `logpath'/main_tab1.rtf, statmat(mat2) sdec(0,0,0,0,0,0 \ 2,2,2,2,2,2\ 2,2,2,2,2,2) ///
title("Stratified by Inst./HCBS") ///
colwidth(20,2,2,2,2,2,2) ///
rtitles("N" \ ///
"Hosp % before adj. (%)" \ ///
"Hosp % after adj. (%)")  ///
ctitles("","Overall","", "2005","","2012","" \ ///
        "","Inst.","HCBS","Inst.","HCBS","Inst.","HCBS") ///
note() ///
replace
wordconvert "`logpath'/main_tab1.rtf" "`logpath'/main_tab1.docx", replace
putdocx append `logpath'/main`ver'.docx `logpath'/main_tab1.docx, saving(`logpath'/main`ver', replace)
...
********************************************************************************
********************************************************************************
*Compare the pqi rate before and after the "hospitalization definition" change
mat mat1=J(3,3,.)
forvalues n=1/3{
preserve
if `n'==1{
...
putdocx pagebreak
putdocx paragraph, style(Heading1)
putdocx text ("Table: PQI Rate Comparison Before and After the “Hospitalization definition” adjustment")
putdocx save `logpath'\main`ver'.docx, append
frmttable using `logpath'/main_tab1.rtf, statmat(mat1) sdec(0,0,0 \ 2,2,2\ 2,2,2) ///
colwidth(20,2,2,2) ///
rtitles("N" \ ///
...
*Balance Check
preserve
collapse (mean) user2005 user2012, by(st_cnty)
gen user_diff=user2012-user2005
egen med_user_diff = median(user_diff)	 
gen med_user_diff_ind=1 if user_diff>med_user_diff
replace med_user_diff_ind=0 if med_user_diff_ind==. 
tab med_user_diff_ind
list
drop user2005 user2012 user_diff med_user_diff
save `data'\user_diff, replace
restore
merge m:1 st_cnty using `data'\user_diff.dta
drop _merge
local varlist "age_cat white_ind female_ind elig_cat dem_ind cc_count_cat10"
local group med_user_diff_ind
sum year 
mat mat1=r(N)
sum year if `group'==0
mat mat2=r(N)
...
putdocx pagebreak
putdocx paragraph, style(Heading1)
putdocx text ("Balance Check by Median - the Percentage of HCBS users among Young Adults (County-level)")
putdocx save `logpath'\main`ver'.docx, append
frmttable using `logpath'/main_tab2.rtf, statmat(mat) sdec(0,0,0 \ 1,1,1) ///
colwidth(20,2,2,2) ///
rtitles("N" \ ///
"Age (%)" \ ///
"     65-69" \ ///
...
"     8-9" \ ///
"     10+" ) ///
ctitles("","Overall","Below Median", "Above Median") ///
note() ///
replace
wordconvert "`logpath'/main_tab2.rtf" "`logpath'/main_tab.docx", replace
putdocx append `logpath'/main`ver'.docx `logpath'/main_tab.docx, saving(`logpath'/main`ver', replace)

********************************************************************************
********************************************************************************
**Sample Size Check for each stratification
forvalues n=1/5{
preserve
if `n'==1{
}
...
mat mat4 = mat1, mat2, mat2[1,1]/mat1[1,1]*100, mat3, mat3[1,1]/mat1[1,1]*100
...
putdocx save `logpath'\main`ver'.docx, append
frmttable using `logpath'/main_tab1.rtf, statmat(mat) sdec(0,0,1,0,1) ///
colwidth(20,2,2,2) ///
rtitles("Overall" \ ///
"White" \ ///
"Black" \ ///
...
"No Dementia" \ ///
"Dementia")  ///
ctitles("","N","Exclusion{\super 1}","%","Exclusion{\super 2}","%") ///
note("Exclude Counties: hcbs%=0/100, hosp%=0/100, pqi%=0/100" \ ///
