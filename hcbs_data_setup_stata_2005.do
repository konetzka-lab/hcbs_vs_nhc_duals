** Datasets created using hcbs_sas_data_setup2005.sas
*takes SAS created datasets, does some basic variable setup
*saves individual state datasets with all benes
*also combined dataset of ltss users in all states
...
clear
use `datapath'\ltc_use_max_2005_`st'_`ds'_2.dta
compress
******************************************************************
...
**save dataset
save `datapath'\ltc_use_max_2005_`st'_`ds'_3.dta, replace
}
...
use `datapath'\ltc_use_max_2005_ak_`ds'_3.dta   
   
foreach st in `statelist'{
	append using `datapath'\ltc_use_max_2005_`st'_`ds'_3.dta
}
...
save `datapath'\ltc_use_max_2005_comb_ltssonly_wdups.dta, replace
...
save `datapath'\max_2005_dups_beneid_only.dta, replace

**now merge merged duplicates back into main dataset
use `datapath'\ltc_use_max_2005_comb_ltssonly_wdups.dta, clear
gen state_cd2=state_cd

drop if beneid_dup_flag==1

append using `datapath'\max_2005_dups_beneid_only.dta
...
save `datapath'\ltc_use_max_2005_comb_ltssonly_`ds'.dta, replace
...
save `datapath'\hcbs_max_analysis_ds_2005.dta, replace
...
use `data'\maxcc_2005.dta if !missing(bene_id), clear
sort bene_id
save `data'\maxcc_2005_benesorted.dta, replace

use `data'\maxcc_2005.dta if missing(bene_id), clear
sort msis_id state_cd
save `data'\maxcc_2005_msissorted.dta, replace
...
clear
import delimited using `mbsf2005'\pqi_2005.csv
...
gen alive`mno'=0
replace alive`mno'=1 if missing(dod_dateformat) //first if no death date, death date after 2005, set to alive=1
replace alive`mno'=1 if (month_died>=`mno' & year_died==2005 & !missing(dod_dateformat) & alive`mno'==0) //now set =1 if died in 2005 but later month
...
gen died_ind=!missing(dod_dateformat) & year_died==2005
...
sort bene_id
save `mbsf2012'\mbsf_2005_sorted.dta, replace




******************************************************************
** 2005 data, drop variables and save  
******************************************************************
use `data_max'\ltc_use_max_2005_comb_ltssonly_all.dta if !missing(bene_id), clear 

******************************************************************
*merge in the max-cc flags
******************************************************************
merge 1:1 bene_id using `data'\maxcc_2005_benesorted.dta
...
******************************************************************
merge 1:1 bene_id using `mbsf2005'\mbsf_2005_sorted.dta
drop if _merge==2
...
save `data'\hcbs_bene_2005.dta, replace
...
*merge in the max-cc flags
merge 1:1 msis_id state_cd using `data'\maxcc_2005_msissorted.dta
...
save `data'\hcbs_msis_2005.dta, replace

******************************************************************
*saves out old version so can import to sas for checks
******************************************************************
**append the two datasets
use `data'\hcbs_bene_2005.dta, clear
append using `data'\hcbs_msis_2005.dta
...
save `data_tk'\hcbs_max_05_iv_sample_v2.dta, replace
******************************************************************
log close
