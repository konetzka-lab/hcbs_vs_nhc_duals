**Alternative IV: county-level percentage of HCBS use among long-term care users aged 65 and older
...
merge m:1 st_cnty using `data'\hcbs_cnty_65_05_12.dta
keep if _merge==3
tab year
local r = `r'+1	
mat s`a'[`r',1]=r(N) 
...
gen iv_cnty65=user2005 if year==2005
replace iv_cnty65=user2012 if year==2012
sum iv_cnty65




local predictor iv_cnty65
local titlelist model-alternativeIV
...
logit `outcome' i.hcbs i.age_cat i.female_ind i.race_5cat c.svc_first_month ///
			 ...
			 i.cc_ra_oa_ind i.cc_other i.cc_anemia_ind i.cc_hyperl_ind ///
			 i.cc_hypert_ind i.cc_hypoth_ind i.ccw_both_alz_dem /// 
			 i.year re `cluster', robust `cluster2'
...
frmttable using `logpath'\main_tab2.rtf, statmat(mat1`outcome'_`title') sdec(3) ///
title("") ///
...
ctitles("Group" "Model" "Variable" "Coef." "p" "95% CI" "" \ ///
		"" "" "" "" "" "Low" "High") ///
replace
wordconvert "`logpath'/main_tab2.rtf" "`logpath'/main_tab2.docx", replace
putdocx append `logpath'/main_`ver'.docx `logpath'/main_tab2.docx, saving(`logpath'/main_`ver', replace)
restore
preserve
...
frmttable using `logpath'\main_tab2.rtf, statmat(mat4`outcome'_`title') sdec(3) ///
title("") ///
