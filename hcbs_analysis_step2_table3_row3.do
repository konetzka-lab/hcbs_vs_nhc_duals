...
drop if _merge==2
drop _merge
merge m:1 bene_id using `data'\pqi_admsn2005.dta	
drop if _merge==2
drop _merge
}
...
gen bene_id2014=bene_id_28773
drop bene_id_28773 bene_id_51206
merge 1:1 bene_id using `data'\admsn2014.dta	
keep if _merge==3
drop bene_id
gen bene_id=bene_id2014
drop bene_id2014 _merge
save  `data'\admsn2014_bene_id, replace

use `data'\walk.dta, replace
gen bene_id=bene_id_28773
gen bene_id2014=bene_id_28773
drop bene_id_28773 bene_id_51206
merge 1:1 bene_id using `data'\pqi_admsn2014.dta	
keep if _merge==3
drop bene_id
gen bene_id=bene_id2014
drop bene_id2014 _merge
save  `data'\pqi_admsn2014_bene_id, replace

use `data'\hcbs_max_`year'_iv_sample_v2.dta, replace
tab year
merge m:1 bene_id using `data'\admsn2014_bene_id.dta	
drop if _merge==2
tab year
drop _merge
merge m:1 bene_id using `data'\pqi_admsn2014_bene_id.dta	
drop if _merge==2
tab year
drop _merge
}
...
************************************************************************
**Variable development and check**
************************************************************************
************************************************************************
...
replace iv_cnty_young=user2014 if year==2014
...
gen hcbs_svc_enroll = mdy(first_wvr_mo, 1, 2014) if year==2014
...
gen hcbs_svc_enroll_last = mdy(last_wvr_mo, 1, 2014) if year==2014
...
merge m:1 st_cnty using `data'\hcbs_cnty_05_14.dta
keep if _merge==3
tab year
local r = `r'+1	
mat s`a'[`r',1]=r(N) 
...
** e9. Counties both in 2005 and 2014
gen flag=1
preserve
...
merge 1:1 st_cnty using `data'\exclusion_cnty_14.dta
keep if _merge==3
gen cnty_flag=1
drop _merge
...
append using `data'\hcbs_14_`ver'
...
putdocx text ("LTSS Analytic Sample restrictions")
putdocx save `logpath'\main`ver'.docx, replace
frmttable using `logpath'/main_tab1.rtf, statmat(full)  sdec(0,1,1,0,1,1) ///
colwidth(20,2,2,2,2,2,2) ///
rtitles("Age 65+ per MBSF"  \ ///
"Full demographic information in MBSF{\super 1}" \ ///
"Race=White, Black" \ ///
"HCBS only or Institutional care only" \ ///
"Exclude transportation only users among HCBS users" \ ///
"Alive for full year" \ ///
"Available State and County Information" \ ///
"Exclude counties with no IV" \ ///
"Counties both in 2005 and 2014" \ ///
"Exclude counties with zero or 100% of HCBS use, hospitalization, PQI Hospitalization" \ ///
"Exclude states: AZ, HI, NM, TN{\super 2}" \ ///
"Exclude states: AR, NJ, OH{\super 3}" \ ///
"Exclude states: AK, CT{\super 4}" \ ///
"Exclude  state, ME{\super 5}")  ///
ctitles("","2005","","","2014","","" \ "","N", "%","%","N","%","%") ///
note("{\super 1}Dropped if missing age, sex, or race from MBSF" \ ///
"{\super 2}States dropped=AZ, HI, NM, TN: Large MLTSS programs." \ ///
"{\super 3}States dropped=AR, NJ, OH: Large change in % of MLTSS btw 05 & 14." \ ///
"{\super 4}States dropped=AK, CT: Diff direction of IV prediction" \ ///
"{\super 5}No ME data in MAX 2005.") ///
replace
...
********************************************************************************
********************************************************************************
**1) Main: County-level Fixed effect 
**2) Main: County-level Fixed effect - with month variable continous variable
**3) Main: County-level Fixed effect - Focus on Janurary people
**4) Main: County-level Fixed effect - Exclude Janurary people
**5) Main: County-level Fixed effect - 60 day hospitalization 
**6) Main: State-level Fixed Effect
**7) Main: No-Fixed Effect
********************************************************************************
********************************************************************************
...
log using `logpath'\model_robust2014.txt, text append

local logpath K:\Outputdata\DJ\logs
local data K:\Outputdata\DJ\data
local predictor iv_cnty_young
local titlelist model103
local clusterlist `""i.st_cnty_num" "i.st_cnty_num" "i.st_cnty_num" "i.st_cnty_num" "i.st_cnty_num" "i.st_num" """'
local clusterlist2 `""cluster(st_cnty_num)" "cluster(st_cnty_num)" "cluster(st_cnty_num)" "cluster(st_cnty_num)"  "cluster(st_cnty_num)" "cluster(st_num)" """'
local word1 woiv
local word2 iv
...
forvalues n2=1/1{
forvalues n=1/2{
local outcome : word `n' of `outcomelist'
...
logit `outcome' i.hcbs i.age_cat i.female_ind i.race_cat c.svc_first_month ///
			 i.elig_ind i.year `cluster', robust `cluster2'
margins, dydx(hcbs)
}
}
}		 
...
preserve
keep if `exclusion'==0
logit hcbs `predictor' i.age_cat i.female_ind i.race_5cat c.svc_first_month  ///
			 i.elig_ind i.year `cluster', robust `cluster2'
...
logit `outcome' i.hcbs i.age_cat i.female_ind i.race_5cat c.svc_first_month ///
			 i.elig_ind i.year `cluster' re, robust `cluster2'
...
local word2 iv
local outcomelist "hosp pqi"
local exclusionlist "exclusion2"
local n_3 : word count `clusterlist'
...
frmttable using `logpath'\main_tab1.rtf, statmat(mat1`outcome'_`title') sdec(3) ///
title("") ///
rtitles("Any Hosp" ,"Without IV", "HCBS" \ ///
		"" ,"With IV", "HCBS" \ ///
		"PQI" ,"Without IV", "HCBS" \ ///
		"" ,"With IV", "HCBS")  ///
...
replace
wordconvert "`logpath'/main_tab1.rtf" "`logpath'/main_tab1.docx", replace
putdocx append `logpath'/main_`ver'.docx `logpath'/main_tab1.docx, saving(`logpath'/main_`ver', replace)
restore
preserve
...
frmttable using `logpath'\main_tab1.rtf, statmat(mat4`outcome'_`title') sdec(3) ///
title("") ///
rtitles("Any Hosp" \ "PQI")  ///
...
preserve
use `data'\dt_hosp_`title'_`word'1.dta, replace
append using `data'\dt_pqi_`title'_`word'2.dta
...
label def group 1 "Any Hosp" 2 "PQI"
la val group group
...
bar(1, color("243 66 85")) bar(2, color("47 127 137")) ///
 over(group, relabel(1 "Any Hosp" 2 "PQI") label(labsize(vsmall))) ///
