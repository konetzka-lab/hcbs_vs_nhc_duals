...
format el_dob date9.;
jan1='01jan2014'd;                                             /*note change this to 2012 when run for 2012 files!*/
...
proc means; variable el_age el_age_years_only; run;
...
/*              Managed LTSS                                               */
/***************************************************************************/
proc freq data=ps_elig; table el_pph_pln_mo_cnt_cmcp el_pph_pln_mo_cnt_: ; run;
...
proc freq data=get_mltss; table mltss_plan_mogt0 pace_plan_mogt0 cmcp_enroll_cat*cmcp_enroll_ind; run;
...
proc means data=clin_cat_2; variables TOT_MDCD_PYMT_AMT
TOT_MDCD_FFS_PYMT_AMT
TOT_MDCD_PREM_PYMT_AMT
FFS_PYMT_AMT_: ;
run;
...
proc freq data=ot_expend_3; table cltc_flag*cat_expend  /missprint ; run;
proc freq data=ot_expend_3; table cltc_flag*hcbs_cltc_nonwaiver_3  /missprint ; run;
...
proc means; variable expend_instltc; run;
...
proc means; variables expend: ; run;
...
data expend_1;
merge clin_cat_2 expend_comb_2 lt_inst_dt ot_hcbs_dt; by msis_id;
...
proc means data=expend_1; variables expend: ; run;

proc means data=expend_1 n mean std min max median; variables TOT_MDCD_PYMT_AMT
TOT_MDCD_FFS_PYMT_AMT
TOT_MDCD_PREM_PYMT_AMT expend_: total_ffs_expend diff diff2; run;

/*
proc means data=expend_1; variables expend_rx ffs_pymt_amt_16; run;

data checkdiff; set expend_1; 
diff_tos_rx = expend_rx - ffs_pymt_amt_16; 
run;

proc means n mean std min max median p01 p99; variable diff_tos_rx; run;
*/
...
proc freq; tables stfips2 state_cd /missprint; run;
...
* filepaths for 2014 data;
%put proc export
data=desc_wk.ltc_use_max_&year._&st._all_2
outfile="\\prfs.cri.uchicago.edu\medicaid-max\Users\jung\data\hcbs\ltc_use_max_&year._&st._all_2.dta" replace;
run;
